# Run 02 — Reduced PKR 1B Funding Round (stress-tests budget constraint)

**Mode:** Demo Mode (deterministic, rule-based reasoning over real tool results)
**Funding envelope:** PKR 1,000M
**Dataset:** 4083 projects, PKR 51,550.1M total portfolio, 39 districts

## Agent Plan & Tool Calls (Agent Activity Panel)

```text
[Coordinator] PLAN: load dataset, dispatch Finance/Delivery/Equity reviews, then rank & select Not Started candidates within the PKR 1000M funding envelope.
...
[Coordinator] ACT: calling select_within_budget() to enforce the PKR 1000M cap and district/category concentration limits.
[Coordinator] OBSERVE: selected 181 projects totaling 999.55M (remaining 0.45M).
...
*(+ 171 more projects in the full run — see CSV export in the app)*
